...
Source groups are `fileSystemSynchronizedGroups`: new `.swift` files under `PinePods/` are picked up automatically without editing `project.pbxproj`.
...
- Shared building blocks live in `Utilities/`:
  ...
  - `Theme.swift`: `PineGreen`, `SkipSymbol` (SF Symbols only has numbered skip glyphs for 5/10/15/30/45/60/75/90 s) and `PlaybackSpeed`.
